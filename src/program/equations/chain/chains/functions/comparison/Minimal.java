...
public class Minimal extends DoubleFormat implements IFunction {
    public static final String syntax = "min";

    public Minimal(IValue val1, IValue val2) {
        super(val1, val2);
    }

    protected Minimal(List<IValue> values) {
        super(values);
    }
    ...
    public Float getValue(float x) {
        ...
        return Math.min(val1.getValue(x), val2.getValue(x));
    }

    @Override
    public IValue getChain(String s) throws IOException {
        return new Minimal(getArguments(s));
    }
